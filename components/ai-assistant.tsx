...
export function AiAssistant() {
  ...
  const generateSuggestions = async () => {
    ...
    try {
      ...
      if (Array.isArray(data.suggestions) && data.suggestions.length > 0) {
        ...
        showFallbackSuggestions("Using default suggestions.")
      }
    } catch (error) {
      console.error("Error generating suggestions:", error)
      toast({
        title: "AI Error",
        description: "Couldn't get suggestions. Showing default ones.",
        variant: "destructive",
      })
  ...
  const showFallbackSuggestions = (msg?: string) => {
    const fallback = [
      "Review your goals for the week",
      "Schedule time for exercise today",
      "Clear your email inbox",
  ...
  const addSuggestionToTasks = async (suggestion: string) => {
    try {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Need inspiration? Let AI suggest some tasks for you.
        </CardDescription>
      </CardHeader>
